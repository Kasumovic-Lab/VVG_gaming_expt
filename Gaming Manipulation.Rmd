...
```{r}
SGAVVG <- lm(SGAAverage ~  VorNV + Age + Gender + Gender*Averagefrequencyviolence + VorNV*Gender + VorNV*Age + VorNV*Averagefrequencyviolence + Averagefrequencyviolence + Selfscore + Selfscore*Gender + Selfscore*Age + Numericalgameplayed +Numericalgameplayed*Gender + Numericalgameplayed*Age +Relationshipstatus + Relationshipstatus*Age, data = data)
...
```{r}
plotMvAverageselfs <- ggplot(data, aes(x = MV2Average, y = Selfscore, color = Gender, shape = Gender, linetype = Gender)) + 
  geom_point(size = 4) + 
  ylim(0,10) +
  scale_colour_hue(l=50, guide = FALSE) +
  scale_x_continuous(limits=c(0.5,7.5), breaks=c(1:7)) +
...
MV1 x selfscore 
```{r}
plotMv2selfs <-ggplot(data, aes(x = MV2Average, y = Selfscore, color = Gender, shape = Gender, linetype = Gender)) + 
  geom_point(size = 4) + 
  ylim(0,7.5) +
  scale_colour_hue(l=50, guide = FALSE) +
  scale_x_continuous(limits=c(0.5,7.5), breaks=c(1:7)) +
...
MV2+  Selfscore*Gender 
```{r}
plotmv2selfscore <- ggplot(data, aes(x = Selfscore, y = MV2Average, color = Gender, shape = Gender, linetype = Gender)) + 
  geom_point(size = 4) + 
  ylim(1,7) +
  scale_colour_hue(l=50, guide = FALSE) +
  scale_x_continuous(limits=c(0,7.5)) +
...
##MANOVA
```{r}
MANOVA1<- manova(cbind(MV1Total, MV2Average, SGAAverage, HostilityTotal, SOIRTOTAL, MATINGMOTIVATIONtotal) ~ VorNV + Age + Selfscore + Gender + Averagefrequencyviolence + Age*Averagefrequencyviolence + Gender*Averagefrequencyviolence + Selfscore*Gender + Age*Gender + Age*Selfscore, data=data)
